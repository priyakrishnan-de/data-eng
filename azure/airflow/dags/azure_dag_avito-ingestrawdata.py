...
with DAG(
    ...
    def ingest_large_tsv_files_with_copy():
        ...
        for file, table in files_to_tables.items():
            ...
            # Absolute or relative path on your VM where the file is stored
            local_file_path = f"/home/azureuser/datasets/{file}"
            ...
            # Create table if not exists (simple schema inference from pandas)
            df = pd.read_csv(local_file_path, sep="\t", nrows=0)
            cols = ", ".join([f'"{col}" TEXT' for col in df.columns])
            cur.execute(f'CREATE TABLE IF NOT EXISTS "{table}" ({cols});')
            conn.commit()

            # Now, open the same temporary file in text mode for reading
            with open(local_file_path, mode='r') as read_file:
