...
def insert_records():
    hook = PostgresHook(postgres_conn_id="postgres_conn") #Azure SQL Postgres
    conn = hook.get_conn()
    cur = conn.cursor()
    ...
    for i in range(1,11):  # insert ~10 rows
        ...
        new_rows.append((
            next_id,   # id
            random.randint(1, int(1e6)),   # searchid
            random.randint(1, int(1e6)),   # adid
...
with DAG(
    "azure_producer_trainsearchstream",
    default_args=default_args,
    schedule_interval="*/10 * * * *",  # every 10 mins
    catchup=False,
    tags=["azure-sql-postgres","producer","continous data simulation-trainsearchstream"],
) as dag:
